...
public class DelphiInterpreter extends delphiBaseVisitor<Value> {
    ...
    public Value visitTypeSection(delphiParser.TypeSectionContext ctx) {
        ...
        env.resolveSuperclasses();
        return Value.nil();
    }

    @Override
    public Value visitClassTypeDecl(delphiParser.ClassTypeDeclContext ctx) {
        String className = ctx.ID().getText();
        ClassDef cd = new ClassDef(className);
        delphiParser.ClassTypeContext ct = ctx.classType();

        if (ct.LPAREN() != null) {
            cd.superclassName = ct.getToken(delphiParser.ID, 0).getText();
        }
        if (ct.IMPLEMENTS() != null) {
            for (var t : ct.idList().ID()) {
                cd.implementsList.add(t.getText());
            }
        }

        currentVis = ClassDef.Vis.PUBLIC;
        delphiParser.ClassMembersContext members = ct.classMembers();
    ...
    @Override
    public Value visitInterfaceTypeDecl(delphiParser.InterfaceTypeDeclContext ctx) {
        String name = ctx.ID().getText();
        InterfaceDef id = new InterfaceDef(name);
        delphiParser.InterfaceMembersContext im = ctx.interfaceType().interfaceMembers();
        if (im != null) {
            for (var imd : im.interfaceMethodDecl()) {
                String methodName = imd.procedureHeader() != null
                    ? imd.procedureHeader().ID().getText()
                    : imd.functionHeader().ID().getText();
                id.methodNames.add(methodName);
            }
        }
        env.interfaces.put(name, id);
        return Value.nil();
    }
    ...
    public Value visitVarSection(delphiParser.VarSectionContext ctx) {
        for (delphiParser.VarDeclContext vd : ctx.varDecl()) {
            ...
            String type = vd.typeName().getText();

            for (String n : names) {
                if (type.equalsIgnoreCase("integer")) {
                    env.declareVar(n, type, Value.ofInt(0));
                } else {
                    env.declareVar(n, type, Value.nil()); // object/interface refs start as nil
                }
            }
        }
        return Value.nil();
    ...
    public Value visitAssignment(delphiParser.AssignmentContext ctx) {
        ...
        if (left.contains(".")) {
            ...
            ClassDef definingClass = obj.klass.getFieldDefiningClass(field);
        ...
        // In method/constructor: bare ID that is a field means self.field (including inherited)
        if (!env.selfCtx.isEmpty()) {
            ClassDef cd = env.classCtx.peek();
            if (cd != null && cd.getFieldDefiningClass(left) != null) {
                env.currentSelf().fields.put(left, rhs);
                return Value.nil();
            }
        }
        ...
        if (name.equals("readln")) {
            ...
            try {
                if (!scanner.hasNextInt()) {
                    if (!scanner.hasNext()) {
                        x = 0; // no input (e.g. non-interactive run): use 0
                    } else {
                ...
                scanner.next(); // consume the bad token
                x = 0;
            }
    ...
    public Value visitMethodOrStaticCall(delphiParser.MethodOrStaticCallContext ctx) {
        ...
        if (env.classes.containsKey(left)) {
            ...
            ClassDef.MethodInfo ctor = cd.getConstructor();

            if (ctor == null) {
                var mi = cd.getMethod(member);
        ...
        ClassDef.MethodInfo mi = cd.getMethod(member);
        if (mi == null || mi.body == null) throw new RuntimeException("Method not implemented: " + cd.name + "." + member);

        ClassDef dtorClass = cd.getMethodDefiningClass(member);
        ClassDef.MethodInfo dtor = dtorClass != null ? dtorClass.destructor : null;
        if (dtor != null && member.equalsIgnoreCase(dtor.methodName)) {
            executeDestructor(dtorClass, obj, dtor, args);
            return Value.nil();
        }
    ...
    public Value visitLvalExpr(delphiParser.LvalExprContext ctx) {
        ...
        if (!env.selfCtx.isEmpty()) {
            ClassDef cd = env.classCtx.peek();
            if (cd != null && cd.getFieldDefiningClass(lv) != null) {
                return env.currentSelf().fields.get(lv);
            }
        }
    ...
    @Override
    public Value visitCallExprAlt(delphiParser.CallExprAltContext ctx) {
    ...
    private void enforceFieldVisibility(ClassDef cd, String field) {
        ClassDef definingClass = cd.getFieldDefiningClass(field);
        if (definingClass == null) return;
        ClassDef.Vis vis = definingClass.fields.get(field);
        if (vis == ClassDef.Vis.PUBLIC) return;
        if (vis == ClassDef.Vis.PRIVATE && (env.classCtx.isEmpty() || env.classCtx.peek() != definingClass))
            throw new RuntimeException("Field access denied (private): " + definingClass.name + "." + field);
        if (vis == ClassDef.Vis.PROTECTED && (env.classCtx.isEmpty() || !env.classCtx.peek().isSubclassOf(definingClass)))
            throw new RuntimeException("Field access denied (protected): " + definingClass.name + "." + field);
    }

    private void enforceMethodVisibility(ClassDef cd, String method) {
        ClassDef definingClass = cd.getMethodDefiningClass(method);
        if (definingClass == null) return;
        ClassDef.Vis vis = definingClass.methodVis.get(method);
        if (vis == null || vis == ClassDef.Vis.PUBLIC) return;
        if (vis == ClassDef.Vis.PRIVATE && (env.classCtx.isEmpty() || env.classCtx.peek() != definingClass))
            throw new RuntimeException("Method access denied (private): " + definingClass.name + "." + method);
        if (vis == ClassDef.Vis.PROTECTED && (env.classCtx.isEmpty() || !env.classCtx.peek().isSubclassOf(definingClass)))
            throw new RuntimeException("Method access denied (protected): " + definingClass.name + "." + method);
    }
}
